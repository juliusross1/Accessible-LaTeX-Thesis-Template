...
This LaTeX has been written as a bare-bones example of something that can be made to work with LaTeXml to produce accessible epub files.   But it is not foolproof, and if authors do exciting and/or unexpected things with LaTeX then it is likely that the result will not be accessible and/or LaTeXml will simply fail.

Some tips

- It is recommended that you regularly run both pdflatex and latexmlc (see below) on this document as you write it.  Do not assume that just because pdflatex is working that the latexmlc also works.
